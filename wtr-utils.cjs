 ...
 */
const _getChangedPackages = () => {
  const pathToLerna = path.normalize("./node_modules/.bin/lerna");
  const output = execSync(
    `${pathToLerna} la --since origin/24.3 --json --loglevel silent`
  ); // NOSONAR
  return JSON.parse(output.toString()).map((project) =>
    project.name.replace("@vaadin/", "")
  );
};
  ...
  return fs
    ...
    .filter(
      (dir) =>
        ...
        glob.sync(`packages/${dir}/test/*.test.{js,ts}`).length > 0
    );
};
...
const getAllSnapshotPackages = () => {
  return fs
    ...
    .filter(
      (dir) =>
        fs.statSync(`packages/${dir}`).isDirectory() &&
        fs.existsSync(`packages/${dir}/test/dom`)
    );
};
...
const getAllVisualPackages = () => {
  return fs
    ...
    .filter(
      (dir) =>
        fs.statSync(`packages/${dir}`).isDirectory() &&
        fs.existsSync(`packages/${dir}/test/visual`)
    );
};
...
const getScreenshotFileName = ({ name, testFile }, type, diff) => {
  ...
  if (testFile.includes("-styles")) {
    const match = testFile.match(
      /\/packages\/(vaadin-(lumo|material)-styles\/test\/visual\/)(.+)/u
    );
...
const getBaselineScreenshotName = (args) =>
  getScreenshotFileName(args, "baseline");

const getDiffScreenshotName = (args) =>
  getScreenshotFileName(args, "failed", true);
...
const createUnitTestsConfig = (config) => {
  ...
  let groups = getUnitTestGroups(testPackages);

  if (!!config.packageFilter?.length) {
    groups = groups.filter((group) => {
      return config.packageFilter.indexOf(group.name) > -1;
    });
  }
...
const createIntegrationTestsConfig = (config) => {
  ...
  // When running in GitHub Actions, do nothing.
  if (
    !changedPackages.includes("integration-tests") &&
