/**
 * Borrowed from @web/dev-server-core/src/plugins/parseDynamicImport.ts
 */
const REGEXP_WHITESPACE = /\s/;
function getDynamicImportStartAndEnd(str, codeStart) {
  let start = -1;
  let end = -1;
...
export function parseDynamicImport(code, start, end) {
  const rawDynamicImport = code.substring(start, end);
  const { dynamicStart, dynamicEnd } = getDynamicImportStartAndEnd(
    rawDynamicImport,
    start
  );
